...
class PublicIterator implements Iterator<PublicChannel> {
	 ...
	 */
	private static class LocationDeserializer implements
	 ...
	 * @param tag
	 *            Get channels with this tag only, or null to return all
	 *            channels.
	 */
	...
	private void thingRequest(Integer page) {
		try {
			...
			results = gson.fromJson(response.getBody().toString(),
					PublicJSONResult.class);
			iterator = results.iterator();
		} catch (UnirestException | ThingSpeakException ex) {
			Logger.getLogger(PublicIterator.class.getName()).log(Level.SEVERE,
					null, ex);
			results = null;
		}
	}
	...
	public boolean hasNext() {
		if (iterator.hasNext()) {
			...
			/*
			 * current page has returned all channels and there are no more
			 * pages left
			 */
			return false;
		} else {
			/*
			 * current page has returned all channels but there are more pages
			 * remaining.
			 */
