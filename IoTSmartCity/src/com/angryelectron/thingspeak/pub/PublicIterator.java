...
/**
 * Provides a custom iterator for PublicChannelCollections which works with ThingSpeak's paginated results.
 *
 * @author abythell
 */
class PublicIterator implements Iterator<PublicChannel> {
	...
	/**
	 * Current page of results and an iterator to the list of PublicChannel channels it contains.
	 */
	private PublicJSONResult results;
	private Iterator<PublicChannel> iterator;
	
	/**
	 * ThingSpeak channels don't always contain elevation, latitude, or longitude data. These empty strings cause
	 * NumberFormatExceptions when using the default GSON de-serializer for Double. This replacement de-serializes empty
	 * strings to 0.0.
	 */
	 ...
	 */
	private final Gson gson = new GsonBuilder().registerTypeAdapter(Double.class, new LocationDeserializer())
	 ...
	 *
	 * @param url
	 *            ThingSpeak server URL (eg. http://api.thingspeak.com).
	 * @param tag
	 *            Get channels with this tag only, or null to return all channels.
	 */
	...
	private void thingRequest(Integer page) {
		try {
			...
			if (response.getCode() != 200) { throw new ThingSpeakException("Request failed with code "
					+ response.getCode()); }
	...
	public boolean hasNext() {
		if (iterator.hasNext()) {
			...
			/*
			 * current page has returned all channels and there are no more pages left
			 */
			return false;
		} else {
			/*
			 * current page has returned all channels but there are more pages remaining.
			 */
