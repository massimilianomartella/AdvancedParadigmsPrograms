...
public class ModelReactiveControlSensor extends Thread {
	...
	public ModelReactiveControlSensor(double temperatureRange, int frequencyTemperature, double humidityRange,
			int frequencyHumidity, StopFlag flag, SensorsView view, MonitorFlag monitorTemperature,
			MonitorFlag monitorHumidity, DataDht11 dht11) {
	...
	public void run() {
		...
		Observable<Double> observableHumidity = Observable.create((Subscriber<? super Double> subscriber) -> {
			new ObservableHumidityStream(subscriber, dht11, flag, frequencyHumidity).start();
		});
		
		Observable<Double> observableTemperature = Observable.create((Subscriber<? super Double> subscriber) -> {
			new ObservableTemperatureStream(subscriber, dht11, flag, frequencyTemperature).start();
		});
		
		// Sensor connection by calling the function that identifies
		// getSpikeValue spikes for each sensor.
		ConnectableObservable<Double> connectObsHumidity = getSpikeValue(observableHumidity, "Humidity", humidityRange,
				thresholdHumidityError).publish();
		ConnectableObservable<Double> connectObsTemperature = getSpikeValue(observableTemperature, "Temperature",
				temperatureRange, thresholdTemperatureError).publish();
		
		Observable.interval(20, TimeUnit.SECONDS, Schedulers.io()).subscribe((Long v) -> {
			if (!flag.isDone()) {
				System.out.println(v);
				ts.sendMsg(1, monitorTemperature.getValue(), 2, monitorHumidity.getValue());
			}
		}, (Throwable t) -> {
		...
		connectObsTemperature.subscribe((Double v) -> {
			// If the value is out of range
				if ((v) < (temperatureRange - thresholdTemperatureError)
					...
					view.setUpdatedMinMaxValueTemperature(monitorTemperature.getMinValue(),
		...
		connectObsHumidity.subscribe((Double v) -> {
			// If the value is out of range
				if ((v) < (humidityRange - thresholdHumidityError) | (v) > (humidityRange + thresholdHumidityError)) {
					// view.setUpdatedJPaneArea("Value out of range > "
					// + v +
					// "\n",
					// Color.red);
					...
					view.setUpdatedMinMaxValueHumidity(monitorHumidity.getMinValue(), monitorHumidity.getMaxValue());
	...
	Observable<Double> getSpikeValue(Observable<Double> stream, String sensor, double temperatureRange,
			double thresholdTemperatureError) {
