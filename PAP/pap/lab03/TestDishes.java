...
public class TestDishes {
	...
	public static void main(String[] args) {
		final List<Dish> menu = Arrays.asList(new Dish("pork", false, 800,
				...
				new Dish("chicken", false, 400, Dish.Type.MEAT), new Dish(
						...
						"prawns", false, 400, Dish.Type.FISH), new Dish(
		...
		List<String> words = Arrays.asList("Hello", "from", "Cesena");
		List<Integer> wordLengths = words.stream().map(s -> s.length())
				.collect(toList());

		wordLengths.stream().forEach((x) -> {
