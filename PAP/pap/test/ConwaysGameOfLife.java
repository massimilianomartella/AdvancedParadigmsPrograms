...
public class ConwaysGameOfLife extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		...
		// URL location =
		// ConwaysGameOfLife.class.getProtectionDomain().getCodeSource().getLocation();
		// System.out.println(location.getFile());
//		game.setIconImage(new ImageIcon(ConwaysGameOfLife.class
//				.getResource("images/logo.png")).getImage());
		game.setSize(DEFAULT_WINDOW_SIZE);
		game.setMinimumSize(MINIMUM_WINDOW_SIZE);
		game.setLocation(
				(Toolkit.getDefaultToolkit().getScreenSize().width - game
						.getWidth()) / 2, (Toolkit.getDefaultToolkit()
						.getScreenSize().height - game.getHeight()) / 2);
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(mi_file_exit)) {
			...
			f_options
					.setLocation(
							(Toolkit.getDefaultToolkit().getScreenSize().width - f_options
									.getWidth()) / 2,
							(Toolkit.getDefaultToolkit().getScreenSize().height - f_options
									.getHeight()) / 2);
			...
			f_autoFill
					.setLocation(
							(Toolkit.getDefaultToolkit().getScreenSize().width - f_autoFill
									.getWidth()) / 2,
							(Toolkit.getDefaultToolkit().getScreenSize().height - f_autoFill
									.getHeight()) / 2);
			...
			Object[] percentageOptions = { "Select", 5, 10, 15, 20, 25, 30, 40,
					50, 60, 70, 80, 90, 95 };
			...
			cb_percent.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (cb_percent.getSelectedIndex() > 0) {
						...
						gb_gameBoard.randomlyFillBoard((Integer) cb_percent
								.getSelectedItem());
						f_autoFill.dispose();
					}
				}
	...
	private class GameBoard extends JPanel implements ComponentListener,
		...
		public void paintComponent(Graphics g) {
			...
			try {
				for (Point newPoint : point) {
					...
					g.fillRect(BLOCK_SIZE + (BLOCK_SIZE * newPoint.x),
							BLOCK_SIZE + (BLOCK_SIZE * newPoint.y), BLOCK_SIZE,
							BLOCK_SIZE);
				}
			...
			for (int i = 0; i <= d_gameBoardSize.height; i++) {
				g.drawLine(BLOCK_SIZE, ((i * BLOCK_SIZE) + BLOCK_SIZE),
						BLOCK_SIZE * (d_gameBoardSize.width + 1),
		...
		public void componentResized(ComponentEvent e) {
			...
			d_gameBoardSize = new Dimension(getWidth() / BLOCK_SIZE - 2,
